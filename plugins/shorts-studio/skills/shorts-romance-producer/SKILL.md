...
`python3 <plugin-root>/scripts/shorts_studio.py romance ...`를 사용합니다. `init → assets 승인 → content 승인 → 검토 렌더 → publish 승인 → 최종 검증·렌더 → upload-package` 순서를 지킵니다.
