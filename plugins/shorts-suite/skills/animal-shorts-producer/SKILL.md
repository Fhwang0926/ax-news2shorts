---
name: animal-shorts-producer
description: Research and compare up to three proven-viral TikTok and YouTube Shorts animal originals, inspect selected frames, create three evidence-grounded Korean story options, and render an approved local Short with source audio, no-vocal music, SFX, CTA, and upload information. Use for animal Shorts discovery and production, not automatic posting, TTS, rights adjudication, or fabricated animal intent.
---
...
Resolve the plugin root two levels above this skill directory. Start with:
    ...
    python3 <plugin-root>/scripts/animal.py doctor --json
...
3. After the user selects one, run init with `--visual-preset observation-contrast-v1` for a fast observation-and-contrast treatment. Only then run acquire for a public URL. Use `animal-viral-card-v1` only when the user asks for the legacy card layout.
   TikTok acquisition uses yt-dlp first and may fall back only to TikTok's official public player in an isolated browser profile. Never reuse login, cookies, or a user browser profile. Read [tiktok/editorial-and-rights.md](references/tiktok/editorial-and-rights.md) before TikTok acquisition.
...
When the user explicitly wants the reviewed TikTok project to continue in the separately installed `whiteboard-shorts` plugin, record a human-reviewed fit score and run:

```text
python3 <plugin-root>/scripts/animal.py whiteboard-handoff \
  --project-dir <project> \
  --fit-score <70..100> \
  --confirm-fit-review
```

Pass the generated `whiteboard-handoff` directory to `whiteboard-shorts`. This compatibility package does not modify Whiteboard Shorts and does not upgrade source rights.
