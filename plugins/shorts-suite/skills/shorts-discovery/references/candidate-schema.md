...
Create one JSON document for each research batch. Timestamps must be ISO 8601 values with a timezone. Public counts must be exact non-negative integers or `null`. Existing inputs default to `candidate_type: video_signal`; use `story_event` only for an event that can be packaged from inspected public evidence without depending on one reusable clip.
  ...
  "candidates": [
    {
      ...
      "inspection": {
        ...
        "packageable_with_public_evidence": true
      },
      "features": {
        "viral_momentum": 95,
        "hook": 92,
      ...
      "feature_reasons": {
        ...
        "entertainment_value": "The strange technique creates anticipation and a satisfying visible result."
      },
      "korean_gap": {
        "queries": [
          "특이한 공구 각도 작업 기술",
...
- `collection_method`: `browser_or_public_web`, `hybrid_youtube_api_browser`
- `candidate_type`: `video_signal`, `story_event`
- `platform`: `youtube`, `tiktok`, `instagram`, `x`, `reddit`, `news`, `community`, `official`, `other`
...
A `story_event` also requires:

```json
{
  "candidate_type": "story_event",
  "subject": "Duolingo",
  "event": "Users received different responses after losing a learning streak.",
  "one_line_payoff": "The company restored the same loss differently depending on the cause.",
  "story_cluster_id": "duolingo-streak-response-2026-08",
  "inspection": {
    "publicly_visible": true,
    "content_observed": true,
    "short_form_or_clippable": false,
    "packageable_with_public_evidence": true
  },
  "package_features": {
    "subject_recognition": 90,
    "conflict_abnormality": 84,
    "payoff_clarity": 92,
    "twist_strength": 88,
    "asset_readiness": 80,
    "freshness": 86,
  ...
  "package_feature_reasons": {
    "subject_recognition": "The subject is a widely recognized consumer app.",
    "conflict_abnormality": "Similar losses received visibly different official responses.",
    "payoff_clarity": "The final contrast can be stated accurately in one sentence.",
    "twist_strength": "The reason for the loss changes the expected company response.",
    "asset_readiness": "Official posts and product visuals can support the sequence.",
    "freshness": "The inspected posts fall inside the requested discovery window.",
    "comment_signal": "Visible reactions show a clear contrast without proving facts.",
    "evidence_strength": "The central facts have official and independent support."
  }
}
```

For `story_event`, `short_form_or_clippable` may be false, but `packageable_with_public_evidence` must be true. `story_cluster_id` deduplicates only records known to show the same event; it is not a broad subject tag.

When YouTube Data API supplied public metadata, use `hybrid_youtube_api_browser` only after browser or equivalent public-page verification. Keep the API signal ID or file path in `research_evidence` or `metric_evidence`; never store the API key.

`features.entertainment_value` and its concrete reason are required for new research batches. Legacy inputs without this field derive it from hook, story twist, and visual clarity, but new work must state why the candidate is actually entertaining rather than merely important or alarming.
