#!/usr/bin/env python3
"""Local tooling for the Shorts Suite healing role.
...
TYPECAST_SETUP_COMMAND = "python3 plugins/shorts-suite/scripts/shorts_suite.py configure-typecast"
...
def download_authorized_source(url: str, destination_dir: Path) -> tuple[Path, dict[str, Any]]:
    ...
    source_request = request.Request(
        checked_url,
        headers={
            "User-Agent": "shorts-suite-healing/0.1 authorized-source-import",
...
def command_init(args: argparse.Namespace) -> int:
    ...
    if args.source_video:
        ...
        temporary_source = tempfile.TemporaryDirectory(prefix="shorts-suite-healing-authorized-source-")
...
def keychain_typecast_api_key() -> str:
    return shared_typecast.api_key_record()[0]
...
def typecast_keychain_check_limited() -> bool:
    return shared_typecast.keychain_check_limited()


@functools.cache
def verified_ssl_context() -> ssl.SSLContext:
    return shared_typecast.verified_ssl_context()
...
def typecast_audio(
    ...
    *,
    ...
    try:
        shared_typecast.synthesize_wav(
            path,
            narration,
            voice_id=voice_id,
            tempo=tempo,
...
def command_render(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix=".shorts-suite-healing-", dir=project_dir) as temp_name:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    init.add_argument("--output-root", default="projects/shorts-suite/healing")
