...
from core.tiktok_public_download import (
    PublicTikTokDownloadError,
    download_public_tiktok_video,
    find_chrome,
)
...
def load_project(project_dir: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if clean_text(project.get("plugin")) not in {"shorts-suite:animal", "animal-viral-shorts"}:
        fail("Shorts Suite animal 프로젝트가 아닙니다.")
...
def command_acquire(args: argparse.Namespace) -> int:
    ...
    acquisition: dict[str, Any] = {"method": "yt-dlp"}
    try:
        run_checked(command, "선택 원본 획득", timeout=900)
    except AnimalViralShortsError as direct_error:
        if clean_text(source.get("platform")) != "tiktok":
            project["status"] = "source_pending"
            project["updated_at"] = now_iso()
            project["source_pending_reason"] = "직접 획득 실패. 권한 있는 로컬 파일이 필요합니다."
            write_json(project_dir / "project.json", project)
            raise
        fallback_path = project_dir / "assets" / "source" / "source-public-player.mp4"
        try:
            result = download_public_tiktok_video(
                url,
                fallback_path,
                max_filesize=args.max_filesize,
                chrome_path=find_chrome(),
            )
        except PublicTikTokDownloadError as fallback_error:
            project["status"] = "source_pending"
            project["updated_at"] = now_iso()
            project["source_pending_reason"] = (
                "yt-dlp와 TikTok 공식 공개 플레이어 획득 실패. 권한 있는 로컬 파일이 필요합니다."
            )
            write_json(project_dir / "project.json", project)
            raise AnimalViralShortsError(
                f"선택 원본 획득 실패: {direct_error}; 공식 플레이어 폴백 실패: {fallback_error}"
            ) from fallback_error
        acquisition = {
            "method": "tiktok-official-public-player",
            "player_url": result.player_url,
            "final_media_host": result.final_media_host,
...
def command_whiteboard_handoff(args: argparse.Namespace) -> int:
    if not args.confirm_fit_review:
        fail("whiteboard 인계에는 사람이 적합성을 검토한 뒤 --confirm-fit-review가 필요합니다.")
    fit_score = float(args.fit_score)
    if not 70 <= fit_score <= 100:
        fail("whiteboard 적합성 점수는 70~100이어야 합니다.")
    project_dir = Path(args.project_dir).expanduser().resolve()
    project, source = load_project(project_dir)
    if clean_text(source.get("platform")) != "tiktok":
        fail("whiteboard 인계는 검토된 TikTok 동물 원본만 지원합니다.")
    source_analysis = require_object(read_json(project_dir / "source-analysis.json"), "source-analysis.json")
    script = require_object(read_json(project_dir / "script.json"), "script.json")
    storyboard = require_object(read_json(project_dir / "storyboard.json"), "storyboard.json")
    preview = require_object(read_json(project_dir / "assets" / "preview" / "preview.json"), "preview.json")
    frames = require_list(preview.get("frames"), "preview.frames")
    if len(frames) < 6:
        fail("whiteboard 인계에는 실제 원본 프레임이 최소 6장 필요합니다.")
    media_path, media_entry = source_media(project_dir, source)
    handoff_dir = project_dir / "whiteboard-handoff"
    if handoff_dir.exists():
        fail(f"기존 whiteboard 인계 폴더를 덮어쓰지 않습니다: {handoff_dir}")
    (handoff_dir / "assets" / "source").mkdir(parents=True)
    (handoff_dir / "assets" / "preview").mkdir(parents=True)
    media_destination = handoff_dir / "assets" / "source" / "source.mp4"
    shutil.copy2(media_path, media_destination)

    compatibility_frames: list[dict[str, Any]] = []
    for index, raw in enumerate(frames, start=1):
        frame = require_object(raw, f"preview.frames[{index - 1}]")
        source_frame = safe_project_path(
            project_dir,
            require_text(frame.get("relative_path"), f"preview.frames[{index - 1}].relative_path"),
            exists=True,
        )
        destination = handoff_dir / "assets" / "preview" / f"frame-{index:03d}{source_frame.suffix.lower()}"
        shutil.copy2(source_frame, destination)
        compatibility_frames.append(
            {
                "relative_path": relative_path(handoff_dir, destination),
                "approx_seconds": frame.get("approx_seconds"),
            }
        )
    preview_payload = {
        "generated_at": now_iso(),
        "source": "assets/source/source.mp4",
        "frames": compatibility_frames,
        "reviewed": True,
        "review_basis": "Shorts Suite animal actual-frame review",
    }
    write_json(handoff_dir / "assets" / "preview" / "preview.json", preview_payload)

    compatibility_scenes: list[dict[str, Any]] = []
    compatibility_segments: list[dict[str, Any]] = []
    source_scenes = require_list(storyboard.get("scenes"), "storyboard.scenes")
    for index, raw in enumerate(source_scenes, start=1):
        scene = require_object(raw, f"storyboard.scenes[{index - 1}]")
        evidence = require_object(scene.get("source_evidence"), f"scene {index}.source_evidence")
        visible = require_list(evidence.get("visible_evidence", []), f"scene {index}.visible_evidence")
        observed_action = clean_text(visible[0] if visible else "")
        if not observed_action:
            fail(f"whiteboard 인계 장면 {index}에 검토된 실제 행동이 필요합니다.")
        role = clean_text(scene.get("role"))
        if index == len(source_scenes):
            role = "conclusion"
        segment_id = f"segment-{index:02d}"
        narration = clean_text(scene.get("caption"))
        headline = require_object(scene.get("headline", {}), f"scene {index}.headline")
        headline_text = " ".join(
            value for value in (clean_text(headline.get("line1")), clean_text(headline.get("line2"))) if value
        )
        compatibility_segments.append(
            {"id": segment_id, "role": role, "korean_narration": narration}
        )
        compatibility_scenes.append(
            {
                "id": clean_text(scene.get("id")) or f"scene-{index:02d}",
                "role": role,
                "script_segment_id": segment_id,
                "korean_narration": narration,
                "korean_caption": narration,
                "duration": scene.get("duration"),
                "source_start_seconds": scene.get("source_start_seconds"),
                "source_clip_seconds": scene.get("source_clip_seconds"),
                "headline": headline_text,
                "source_evidence": {"observed_action": observed_action},
                "animal_emotion": scene.get("animal_emotion", {}),
            }
        )

    raw_rights = require_object(source.get("rights"), "source.rights")
    raw_status = clean_text(raw_rights.get("status"))
    permission_status = raw_status if raw_status in {
        "owned", "licensed", "permission_confirmed", "review_required", "unknown", "not_permitted"
    ...
    compatibility_source = {
        ...
        "rights": {
            ...
            "permission_reference": clean_text(raw_rights.get("evidence")),
        },
        "source_media": [
            {
                "id": "source-video-01",
                "relative_path": "assets/source/source.mp4",
                "sha256": sha256_file(media_destination),
                "probe": media_entry.get("probe"),
            }
        ],
        "analysis_preview": {
            "relative_path": "assets/preview/preview.json",
            "frame_count": len(compatibility_frames),
        },
        ...
        "candidate_snapshot": {
            "format_fit": {"whiteboard": {"score": fit_score, "reviewed": True}}
        },
    }
    write_json(
        handoff_dir / "project.json",
        {
            "plugin": "shorts-suite:whiteboard-handoff",
            "title": clean_text(project.get("project_id")) or project_dir.name,
            "status": "reviewed",
            "created_at": now_iso(),
        },
    )
    write_json(handoff_dir / "source.json", compatibility_source)
    write_json(
        handoff_dir / "viral-analysis.json",
        {
            "status": "reviewed",
            "transcript": {"reviewed": True, "basis": "actual-frame review; no ASR"},
            "source_analysis": source_analysis,
        },
    )
    write_json(
        handoff_dir / "script.json",
        {"segments": compatibility_segments, "source_story_id": script.get("story_id")},
    )
    write_json(
        handoff_dir / "storyboard.json",
        {"scenes": compatibility_scenes, "source_story_id": storyboard.get("story_id")},
    )
    write_json(
        handoff_dir / "rights-manifest.json",
        {
            "source": {
                "permission_status": permission_status,
                "permission_reference": clean_text(raw_rights.get("evidence")),
            ...
            "source_rights_status": raw_status,
    ...
    print(
        json.dumps(
            {
                "status": "whiteboard_handoff_ready",
                "project_dir": str(handoff_dir),
                "whiteboard_modified": False,
                "publication_ready": False,
            },
            ensure_ascii=False,
            indent=2,
...
def command_doctor(args: argparse.Namespace) -> int:
    ...
    payload = {
        ...
        "tiktok_public_player_fallback": {
            "chrome": str(find_chrome() or ""),
            "login_or_cookie_reuse": False,
        },
        "pillow": pillow,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    acquire = subparsers.add_parser("acquire", help="선택된 정식 URL을 yt-dlp 또는 TikTok 공식 공개 플레이어로 로컬 획득합니다.")
    acquire.add_argument("--project-dir", required=True, help="Shorts Suite animal 프로젝트")
    ...
    preview.add_argument("--project-dir", required=True, help="Shorts Suite animal 프로젝트")
    ...
    observe.add_argument("--project-dir", required=True, help="Shorts Suite animal 프로젝트")
    ...
    stories.add_argument("--project-dir", required=True, help="Shorts Suite animal 프로젝트")
    ...
    select_story.add_argument("--project-dir", required=True, help="Shorts Suite animal 프로젝트")
    ...
    compose.add_argument("--project-dir", required=True, help="Shorts Suite animal 프로젝트")
    ...
    edit_plan.add_argument("--project-dir", required=True, help="Shorts Suite animal 프로젝트")
    ...
    validate.add_argument("--project-dir", required=True, help="Shorts Suite animal 프로젝트")
    ...
    approve.add_argument("--project-dir", required=True, help="Shorts Suite animal 프로젝트")
    ...
    render.add_argument("--project-dir", required=True, help="Shorts Suite animal 프로젝트")
    ...
    upload_package.add_argument("--project-dir", required=True, help="Shorts Suite animal 프로젝트")
    upload_package.set_defaults(handler=command_upload_package)

    whiteboard = subparsers.add_parser(
        "whiteboard-handoff",
        help="검토된 TikTok animal 프로젝트를 기존 whiteboard-shorts 입력 계약으로 인계합니다.",
    )
    whiteboard.add_argument("--project-dir", required=True, help="Shorts Suite animal 프로젝트")
    whiteboard.add_argument("--fit-score", required=True, type=float, help="사람이 검토한 whiteboard 적합성 점수")
    whiteboard.add_argument("--confirm-fit-review", action="store_true")
    whiteboard.set_defaults(handler=command_whiteboard_handoff)
    return parser
